    ...
    def challenge(self,
        ...
        if defeat:
            msg = '{}对boss造成了{:,}点伤害，击败了boss\n（{}日第{}刀，{}）'.format(
                nik, health_before, '昨' if previous_day else '今', finished+1, '尾余刀' if is_continue else '收尾刀'
            )
        else:
            msg = '{}对boss造成了{:,}点伤害\n（{}日第{}刀，{}）'.format(
                nik, damage, '昨' if previous_day else '今', finished+1, '剩余刀' if is_continue else '完整刀'
            )
        status = BossStatus(
            group.boss_cycle,
            group.boss_num,
