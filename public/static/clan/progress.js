...
var vm = new Vue({
    ...
    mounted() {
        ...
        thisvue.screenWidth = window.screen.width;
        console.log(thisvue.screenWidth)
    },
    methods: {
        get_today: function () {
            let d = new Date();
        ...
        csummary: function (cha) {
            ...
            return `(${cha.cycle}-${cha.boss_num}) <span class="digit${cha.damage.toString().length}">${cha.damage}</span>`;
        },
        csummaryxs: function (cha1, cha2) {
            if (cha1 == undefined) {
                return '<span>暂无记录</span>';
            }
            if (cha1 && cha1.health_ramain !=0) {
                return `<span>(${cha1.cycle}-${cha1.boss_num}) <span class="digit${cha1.damage.toString().length}">${cha1.damage}</span> (完整刀)</span>`;
            }
            if (cha2 == undefined) {
                return `<span>(${cha1.cycle}-${cha1.boss_num}) <span class="digit${cha1.damage.toString().length}">${cha1.damage}</span> (尾刀)</span>`;
            }
            return `<span>(${cha1.cycle}-${cha1.boss_num}) <span class="digit${cha1.damage.toString().length}">${cha1.damage}</span> (尾刀)</span></br><span>(${cha2.cycle}-${cha2.boss_num}) <span class="digit${cha2.damage.toString().length}">${cha2.damage}</span> (补偿刀)</span>`;
        },
        ...
        report_day: function (date) {
            let reportDate = moment(date).toDate();
            var thisvue = this;
            axios.post('../api/', {
                action: 'get_challenge',
                csrf_token: csrf_token,
                ts: (reportDate ? (reportDate.getTime() / 1000) + 43200 : null),
            }).then(function (res) {
                if (res.data.code != 0) {
                    thisvue.$alert(res.data.message, '获取记录失败');
                } else {
        ...
        refresh: function (challenges) {
            ...
            thisvue.update_member_info(m);
            console.log(thisvue.progressData);
        },
        viewTails: function () {
            this.tailsData = [];
        ...
        handleTitleSelect({ item, key, selectedKeys }) {
            switch (key) {
                case '1':
                    window.location = '../';
                    break;
