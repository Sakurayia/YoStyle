    ...
    mounted() {
        ...
        axios.get("../../assets/data.json").then(function (res) {
            if (res.data.code == 0) {
                ...
                let deadline = new Date(data.year, data.month, data.day, data.hour, data.minute);
                thisvue.deadline = deadline.getTime();
            } else {
                thisvue.$message.error("获取时间信息失败");
            }
        }).catch(function (error) {
            thisvue.$message.error("获取时间信息失败");
        })
    ...
    methods: {
        ...
        recorddamage: function (event) {
            this.callapi({
                ...
                damage: this.damage,
                message:this.message,
                previous_day: this.previous_day,
            });
